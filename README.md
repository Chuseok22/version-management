...
A. `version_bumped: false`인 실행에서는 값이 비어 있는 것이 정상입니다. 버전이 증가했는데도 비어 있다면, `chuseok22/version-management@v1`이 참조하는 floating 태그가 이동 중인 극히 드문 타이밍일 수 있습니다 — 이 경우 `repository_dispatch`의 `sha`는 자동으로 `github.sha`로 폴백됩니다.
