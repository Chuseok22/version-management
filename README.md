...
- **두 프레임워크 지원**
    - **Spring Boot(Gradle Groovy)**
        - `build.gradle` 내 `version = 'X.Y.Z'` 갱신
        - (옵션) `src/main/resources/application.yml`의 `version:` 키 치환
    - **Next.js(TypeScript)**
        - `package.json.version` 갱신
        - `src/constants/version.ts`(경로 커스터마이즈 가능) 생성/치환
...
- **정책 보장**
    ...
    - `CHANGELOG.md`를 **상단 prepend** (최초 생성 시 상단 배너 1회 추가)
    - **Git Tag**(`vX.Y.Z`) 생성·푸시 + **릴리즈 커밋** 푸시
    - 릴리스 커밋 메시지에 **`[skip version]`** 자동 포함(재실행 루프 방지)
    - **버전 증가가 없으면 성공(Success)으로 종료** → 다른 워크플로우 분기 용이
...
│           └─ create-tag.mjs             # 태그 생성/푸시 + 릴리즈 커밋 조정
└─ README.md
```

> 왜 분리했나요?  
> **재사용 워크플로**는 파이프라인 오케스트레이션(권한, 동시성, 브랜치/트리거, 후속 dispatch),  
> **컴포지트 액션**은 실제 로직(버전 계산/파일 수정/태깅) 재사용을 담당합니다.

---

## 🧭 빠른 시작 (소비자 레포)

### 1) 중앙 “재사용 워크플로” 통째로 사용 (권장)
...
> 중앙 레포는 **태그로 고정**해 사용하세요: `@v1` (업그레이드 시 새 태그로 마이그레이션)

### 2) (고급) 기존 CI 내부에서 **로직만** 사용 (컴포지트 액션)

컴포지트 액션은 **이벤트 디스패치(후속 트리거)는 포함하지 않습니다.**  
필요하면 ①번 재사용 워크플로를 사용하세요.
  ...
  only-bump:
    ...
    steps:
      - uses: actions/checkout@v4
        with: { fetch-depth: 0 }     # 태그/히스토리 읽으려면 0 권장
...
| `default_version` | `0.0.0` | 태그/파일 모두 없을 때 시드 버전 |
...
**컴포지트 액션** `.github/actions/version-bump/action.yml` 도 동일/유사 입력을 받습니다(단, dispatch 기능은 워크플로 측에서 제공).

---

## 🔗 후속 워크플로우 예시 (apk-build)

버전이 실제로 증가했을 때만 **자동 트리거**:
...
- **최초 생성 시 1회** 아래 **배너**가 **맨 위**에 들어갑니다:
  ```
  <!-- vm-banner-start -->
  ...
  <!-- vm-banner:end -->
  ```
- 릴리스 커밋 메시지에는 **항상 `[skip version]`** 토큰을 포함해 재실행 루프를 방지합니다.
...
- **동일 레포 내** 태깅/푸시는 **기본 `GITHUB_TOKEN`** 으로 충분합니다.  
  (다른 저장소로 `repository_dispatch` 를 보내려면 PAT 필요)
- `actions/checkout@v4` 는 기본적으로 자격 증명을 설정합니다.  
  태그/히스토리 활용을 위해 `fetch-depth: 0` 권장.
...
- [ ] 기본 브랜치(`main`) 확인
- [ ] 커밋 **제목(subject)** 규칙 확인 (`version(major|min|patch): ...`)
- [ ] 태그가 없을 때 `default_version` 값 확인
- [ ] Spring: `build.gradle` 존재 / Next: `package.json` 존재
- [ ] Next 상수 경로(`next_constants_path`) 확인 (`src/constants/version.ts` 등)
- [ ] 모노레포라면 `workdir` 지정
- [ ] `CHANGELOG.md` 최초 배너 1회 삽입 확인
- [ ] 릴리스 커밋 메시지에 `[skip version]` 포함
...
이슈/PR 환영합니다.  
Spring 멀티모듈 전파, GitHub Release 생성 자동화, 배너 커스터마이즈 등 아이디어도 환영합니다.
